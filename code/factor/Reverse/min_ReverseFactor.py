...
def calc_change(close:pd.Series, ) -> float:
    '''
    计算max/min rise/decline
    '''
    try:
        rtns = (close/close.shift(1)).dropna()
        cum = rtns.cumprod()
        symb = rtns > 1
        pos = ( symb - symb.shift(1)).shift(-1).fillna(1- 2*symb.iloc[-1])

        cum_st_rise =cum[:-1][pos[:-1] == 1]
        cum_en_rise = cum[ (pos == -1) & (cum.index > cum_st_rise.index[0]) ]

        cum_st_decl =cum[:-1][pos[:-1] == -1]
        cum_en_decl = cum[ (pos == 1) & (cum.index > cum_st_decl.index[0]) ]


        # max_rise =(cum_en_rise.values /cum_st_rise.values).max()
        # min_rise =(cum_en_rise.values /cum_st_rise.values).min()

        # max_decline =(cum_st_decl.values / cum_en_decl.values ).max()
        # min_decline =(cum_st_decl.values / cum_en_decl.values ).min()

        max_rise =np.quantile((cum_en_rise.values /cum_st_rise.values),0.9)
        min_rise =np.quantile((cum_en_rise.values /cum_st_rise.values),0.1)

        max_decline =np.quantile(cum_st_decl.values / cum_en_decl.values, 0.9)
        min_decline =np.quantile(cum_st_decl.values / cum_en_decl.values, 0.1)

        reverse = max_rise - min_decline # 越大收益率越小，负向因子
        momentum = max_decline - min_rise # 越大收益率越小，负向因子

        # return - 1/min_decline
        return abs(reverse * momentum)
    except:
        return np.nan
...
def multic_calc(date, result_ls, stk_rtns_columns):
    ...
    # rtns = (close/close.shift(1)).dropna(how='all',axis=0)
    # symb = rtns > 1
    # cum = rtns.cumprod(0)
    # indic = (symb - symb.shift(1)).shift(-1).fillna(1 - 2*symb.iloc[-1]) # 1 代表此刻为最后一次下跌，-1 代表此刻为最后一次上涨
    # cum_st = cum[indic[:-1] == 1]
    # st_time = cum_st.notna().idxmax()
    # index_matrix = np.tile(cum_st.index.values.reshape(-1, 1),(1,cum_st.shape[1]))
    # adj_matrix = (index_matrix > st_time.values)
    # adj_matrix = np.where(adj_matrix==0, np.nan, adj_matrix)

    # cum_en = cum[indic==-1] * adj_matrix
    # rise = [cum_en[stock].dropna().values / cum_st[stock].dropna().values for stock in cum_st.columns]
    # max_rise = [max(ls) if ls.size > 0 else 0 for ls in rise]
    # value = pd.Series(max_rise, index = stk_rtns_columns)


    value = close.apply(calc_change)
...
def main():
    ...
    dates =stk_rtns.index.strftime('%Y%m%d')    
    ...
    factor_initial.to_pickle('./result/factor/Reverse/min_pro_Reverse_Factor.pkl')

    # market_neutral
    factor_tmp_mkt = mkt_neutralize_ls(factor_initial, zz500, hs300, zz1000, all_stock)
    factor_tmp_mkt.to_pickle('./result/factor/Reverse/MN_min_pro_Reverse_Factor.pkl')
